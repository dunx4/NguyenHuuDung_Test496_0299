import requests
from scapy.all import ARP, Ether, srp

def local_network_scan(ip_range):
    # Tạo gói tin ARP request để hỏi "Ai có địa chỉ IP này?"
    arp = ARP(pdst=ip_range)
    # Tạo gói tin Ethernet để gửi quảng bá (broadcast) đến tất cả thiết bị
    ether = Ether(dst="ff:ff:ff:ff:ff:ff")
    packet = ether/arp
    
    # Gửi gói tin và nhận phản hồi (timeout 3 giây)
    result = srp(packet, timeout=3, verbose=0)[0]

    devices = []
    for sent, received in result:
        devices.append({
            'ip': received.psrc,
            'mac': received.hwsrc,
            'vendor': get_vendor_by_mac(received.hwsrc)
        })

    return devices

def get_vendor_by_mac(mac):
    try:
        # Sử dụng API macvendors để tra cứu tên nhà sản xuất
        response = requests.get(f"https://api.macvendors.com/{mac}")
        if response.status_code == 200:
            return response.text
        else:
            return "Unknown"
    except Exception as e:
        print("Error fetching vendor information:", e)
        return "Unknown"

def main():
    # Thay đổi dải IP phù hợp với mạng của bạn (ví dụ: 192.168.1.1/24)
    ip_range = "192.168.88.0/24"
    devices = local_network_scan(ip_range)

    print("Devices on the local network:")
    for device in devices:
        print(f"IP: {device['ip']}, MAC: {device['mac']}, Vendor: {device['vendor']}")

if __name__ == '__main__':
    main()